...
export class ViewsByCategoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    this._dashboard.getCategorList().subscribe((res: any) => {
     ...
     this.last35DaysDate =  this.weeklyDate = this.changeDateToWeek(date.start, date.end);
  ...
  createTable(date: [], categoryArray: []) {
    ...
    this.getAllWeeksData(date, categoryArray).then((res) => {
      ...
      for (let play of res) {
        this.weeklyDataArray[play.Category].push(play.plays);
      }
    });
  }
    ...
    this.themeSubscription = this.theme.getJsTheme().subscribe((config) => {
      ...
      let selectedChannel = selected;
        ...
        if (selected.length > 0) {
          ...
          selectedChannel = this.allCategories;
        }
